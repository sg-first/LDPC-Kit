...
private:
        ...
        const static unsigned int pExp = 3; //2^3=8
        const static unsigned int maxP=(pExp-1)*2+1;
        const static unsigned int ppHighSub=pExp; //x3=1000，下标为3，就是3
        const static unsigned int primitivePolynomials=3; //x+1=11=3
...
public:
        ...
        static std::bitset<maxP> easyMul(std::bitset<maxP> b, unsigned int pos)
        {
            std::bitset<maxP> resultB(0);
        ...
        static void ppMod(std::bitset<maxP> &b)
        {
            for(unsigned int i=maxP-1;i>=ppHighSub;i--)
            {
                if(b[i]==1)
                {
                    b[i]=0;
                    //先用pp推对应的式子
                    std::bitset<maxP> pp(primitivePolynomials);
        ...
        static unsigned int rawMul(unsigned int a,unsigned int b)
        {
            ...
            std::bitset<maxP> ab(a);
            std::bitset<maxP> bb(b);
            unsigned int size=ab.size();

            std::vector< std::bitset<maxP> > result;

            for(unsigned int i=0;i<size;i++)
            {
                if(ab[i]==1)
                {
                    std::bitset<maxP> newr=easyMul(bb,i);
                    ppMod(newr);
        ...
        static void initMulTable()
        {
            ...
            for(unsigned int i=0;i<p;i++)
                for(unsigned int j=0;j<p;j++,num++)
                {
                    mulTable[num][0]=i;
                    mulTable[num][1]=j;
                    mulTable[num][2]=vector::rawMul(i,j);
                }
        }
...
public:
        ...
        {
            double d=this->det();
            return this->adjoint().mul(vector::mulInv(d));
        }
